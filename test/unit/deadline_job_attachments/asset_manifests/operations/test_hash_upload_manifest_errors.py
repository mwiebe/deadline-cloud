...
class TestPrintFunctionCallback:
    ...
    def test_print_callback_called_on_hash_upload(self, tmp_path: Path) -> None:
        """Test that print_function_callback is called during processing."""
        cache_root = tmp_path / "cache"
        ...
        hash_upload_manifest(
            manifest=manifest,
            data_cache=data_cache,
            print_function_callback=capture_callback,
        )

        # Should have received at least one message about hashing/uploading
        assert len(messages) > 0
        assert any("Hashed" in msg or "uploaded" in msg for msg in messages)

    def test_print_callback_called_for_cached_files(self, tmp_path: Path) -> None:
        """Test that print_function_callback is called for cached files."""
        cache_root = tmp_path / "cache"
        hash_cache_dir = tmp_path / "hash_cache"
        hash_cache_dir.mkdir()
        ...
        with HashCache(str(hash_cache_dir)) as hash_cache:
            ...
            # Should have message about cached file
            assert any("cached" in msg.lower() for msg in messages)
