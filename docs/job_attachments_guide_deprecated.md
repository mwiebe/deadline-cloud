# Job Attachments Guide (deprecated)

See the in-progress [Job Attachments Guide](job_attachments_guide.md) for new content.
...
You can then easily download your outputs with the [deadline cli](cli_reference/index.md) `deadline job download-output` command, or using the [protocol handler](#protocol-handler) to download from a click of a button in the [AWS Deadline Cloud monitor][monitor].
